...
describe('Register page test', () => {
  ...
  it('should create proper request and response of 201 CREATED', () => {
    ...
    cy.get('#username').click().type(usedUsername);
    ...
    // fields should all be cleared
    cy.get('#passwordConfirmation').should('have.value', '');
    cy.get('#username').should('have.value', '');
    cy.get('#password').should('have.value', '');
  });

  it('should create proper req/res of 400 BAD REQUEST with proper alert', () => {
    expect(usedUsername).to.not.null
    expect(usedPassword).to.not.null
    // repeat registration
    cy.get('#username').click().type(usedUsername);
    cy.get('#password').type(usedPassword);
    cy.get('#passwordConfirmation').type(usedPassword);
    ...
    cy.on('window:alert', (alertText) => {
      expect(alertText).to.equal(
        'UserExceptions: Username name has already been taken.'
      );
    });
  });
});
